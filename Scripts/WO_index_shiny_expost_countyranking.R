...
##
## expost_countyranking: The county data will be normalized for each district
##                       clicked and only across the counties within the 
##                       district.
##
## Author: Gerrit Stahn
##
...
server <- function(input, output, session) {
    ...
    shinyalert(
      ...
      <div style='text-align:left; line-height:1.4;'>
        ...
        <ul>
          ...
          <li>You can <b>click on any district</b> in the chart to explore it in more detail.</li>
          <li>After clicking, a second chart appears showing a <b>ranking of municipalities within that district</b>, allowing you to identify the most suitable place.</li>
        </ul>
        ...
        <p style='margin-top:10px;'>
          <i>Tips:</i> If you leave sliders at 0, those factors will not affect the ranking.
          Try different weights to see which regions fit different lifestyles.
        </p>
      </div>
    "),
 ...
 ### GEM data ###
  county_data_reactive <- reactive({
    ...
    raw_county_data <- read_rds(
      file = paste0(path_data, "/Manipulated/data_gem_2023.rds")
    ) %>%
      select(-Unemp_Men)
    
    district_id <- selected_district()$ID_K
    
    # Nur Gemeinden des geklickten Kreises
    ...
    # Nur die Variablen behalten, die tatsächlich im Datensatz vorkommen
    order_vars <- order_vars[order_vars %in% names(county_data)]
    weights <- weights[order_vars]
    
    # Innerhalb des geklickten Kreises normalisieren
    ...
    top_synth_county <- sum(weights)
    
    county_index <- county_data_norm %>%
      relocate(all_of(order_vars), .after = "Name") %>%
      rowwise() %>%
      mutate(
        Index = round((sum(c_across(all_of(order_vars)) * weights, na.rm = TRUE) / top_synth_county) * 100)
      ) %>%
      ungroup() %>%
